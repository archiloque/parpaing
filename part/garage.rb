class Garage
  ...
  def create_front_facing()
    ...
    create_windows(
      @house_definition.windows_type[:back].new,
      @z_width - 1
    )
  ...
  def create_windows(windows_part, windows_z)
  ...
  def create_back_facing()
    concat_result(
      [
        Emitter.emit(
          ...
          x: (@x_origin * BRICK_WIDTH) + 80,
          y: -100,
          z: ((@z_origin + @z_width - 1) * BRICK_WIDTH),
        ),
        Emitter.emit(
          part: DOOR_RIGHT,
          color: @house_definition.door_and_windows_color,
    ...
    create_windows(
      @house_definition.windows_type[:front].new,
      0
    )
    create_roof
    0.downto(-DOOR_HEIGHT + 1) do |row|
